...
let do_game counts num_steps =
    let rows, cols = get_matrix_dims counts in
    let rec loop i acc counts =
        Printf.printf "i=%d acc=%d\n" i acc;
        if i = num_steps then acc
        else begin
            ...
            (* Printf.printf "total_flashes=%d counts'=%s\n" total_flashes (format_matrix_of_int
               counts'); *)
            loop (succ i) (acc + total_flashes) counts'
        end
    in
    loop 0 0 counts
;;
...
let process_file filename =
    ...
    let result = do_game chunks 100 in

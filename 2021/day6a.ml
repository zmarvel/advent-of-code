...
let do_game ages days =
    let rec day_loop i ages =
        if i = days then
            ...
            let rec age_loop j num_new =
                if j = Array.length ages then
                    num_new
                else
                    let age = ages.(j) in
                    let age' = if age = 0 then 6 else pred age in
                    ages.(j) <- age';
                    age_loop (succ j) (if age = 0 then (succ num_new) else num_new)
            in
            let num_new = age_loop 0 0 in
            day_loop (succ i) (Array.append ages (Array.make num_new 8))
    in
    Array.length (day_loop 0 ages)
;;
...
let process_file filename =
    ...
    let ages = load_file inc in
    let answer = do_game ages 80 in

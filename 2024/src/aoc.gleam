...
pub fn find_total_distance(left: List(Int), right: List(Int)) -> Int {
  ...
  let distances = list.map2(left_sorted, right_sorted, fn (a, b) {
...
pub fn find_similarity_score(left: List(Int), right: List(Int)) -> Int {
  ...
  let right_counts = right
    |> list.fold(dict.new(), fn (acc, x) {
      dict.upsert(acc, x, increment)
    })
  let assert Ok(similarity_score) = left
  |> list.map(fn (x) {
...
pub fn main() {
  ...
  io.println("Total distance: " <> {
    find_total_distance(left, right) |> int.to_string
  })
  io.println("Similarity score: " <> {
    find_similarity_score(left, right) |> int.to_string
  })
}
